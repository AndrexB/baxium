...
This guide describes how to build baxiumd, command-line utilities, and GUI on macOS.
...
It is not necessary to build wallet functionality to run `baxiumd` or  `bitcoin-qt`.
...
Bitcoin Core should now be available at `./build/bin/baxiumd`.
...
The first time you run `baxiumd` or `bitcoin-qt`, it will start downloading the blockchain.
...
```shell
./build/bin/baxiumd -daemon      # Starts the bitcoin daemon.

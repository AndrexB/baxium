...
`./`               | `bitcoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `baxiumd` or `bitcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `baxiumd.pid`        | Stores the process ID (PID) of `baxiumd` or `bitcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `baxiumd` or `bitcoin-qt`; can be specified by `-debuglogfile` option
